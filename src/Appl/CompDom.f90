...
!          private
      ...
      contains
        ...
        elseif(flg.eq.6) then !read in
          ...
          !zmin = xmu5 -sqrt(3.0)*sig11z/sqrt(1.0-uz*uz)
          !zmax = xmu5 + sqrt(3.0)*sig11z/sqrt(1.0-uz*uz)
          xmin = xmu1 -sqrt(3.0)*sig11x
          xmax = xmu1 + sqrt(3.0)*sig11x
          ymin = xmu3 -sqrt(3.0)*sig11y
          ymax = xmu3 + sqrt(3.0)*sig11y
          zmin = xmu5 -sqrt(3.0)*sig11z
          zmax = xmu5 + sqrt(3.0)*sig11z
        ...
        if(nproccol.gt.1) then
          if((Flagbc.eq.3).or.(Flagbc.eq.4)) then
            ...
            if(flg.eq.2) then
              ...
              interval0 = sumint/dble(nproccol)
              interval1 = interval0
              sumint = 0.0
...
!        enddo
        ...
        integer, intent(in) :: npz,npy,commrow,commcol,innx,inny,innz,&
                               inyglb,inzglb
...
!        interval0 = 1.0/dble(npz)
        interval0 = sum(rhoznew)/dble(npz)
        interval1 = interval0 
        icount = 0
...
!        interval0 = 1.0/dble(npy)
        interval0 = sum(rhoynew)/dble(npy)
        interval1 = interval0 
        icount = 0
...
!        temp = sum(recvrhoy)
        ...
        integer, intent(in) :: npz,npy,commrow,commcol,innx,inny,&
                               innz,inyglb,inzglb
        integer, dimension(0:npz-1), intent(inout) :: lctabnmz
        double precision, dimension(2,0:npz-1), intent(out) :: lctabrgz
        integer, dimension(0:npz-1) :: ztable,zdisp
...
!        endif
        ...
        !nzloctest = lctabnmz(myranktest)
        !print*,"nzloctest: ",myranktest,nzloctest
        do i = 0, npz - 1
          if(lctabnmz(i).eq.0) then
            lctabnmz(i) = 1
            lctabrgz(1,i) = lctabrgz(1,i) - hz
